...
motion_history = cv2.createBackgroundSubtractorMOG2()
...
while True:
    ...
    fg_mask_motion = motion_history.apply(frame)

    # Apply background subtraction for person detection
    fg_mask_people = bg_subtractor.apply(frame)

    # Combine motion and person detection masks
    combined_mask = cv2.bitwise_or(fg_mask_motion, fg_mask_people)
    ...
    if people_boxes is not None and not person_motion_detected:
        for (x, y, w, h) in people_boxes:
            ...
            if cv2.countNonZero(combined_mask[y:y + h, x:x + w]) > motion_threshold:
                # Play gunshot sound
                gunshot_sound.play()
