...
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # Reduce resolution for faster processing
...
# Flag to track if motion of a person is detected
...
while True:
    ...
    people_boxes, _ = hog.detectMultiScale(frame, scale=1.05, winStride=(4, 4))
    ...
    if people_boxes is not None:
        for (x, y, w, h) in people_boxes:
            ...
            if cv2.countNonZero(roi) > motion_threshold:
                # Play gunshot sound
                gunshot_sound.play()
